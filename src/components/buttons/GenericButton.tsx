...
const GenericButton: React.FC<Props> = ({
  ...
  if (theme === 'teal') {
    ...
    return (
      <button
        className={
          'overflow-hidden gap-2.5 self-stretch px-11 py-4 text-base font-bold text-white bg-indigo-500 rounded-b-xl rounded-tl-xl w-full transition-colors hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed'
        }
        onClick={onClick}
      >
        {label}
